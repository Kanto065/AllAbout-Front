...
const CreateUser = () => {
  ...
  const location = useLocation(); // Import useLocation to access `location`

  const State = location?.state;

  useEffect(() => {
    // Ensure State is used appropriately
    if (State) {
      console.log("State available:", State);
      // Perform any other necessary actions with State
    }
  }, [State]);
  ...
  const handleFacebookLogin = async () => {
    try {
      ...
      Swal.fire({
        title: "Error!",
        text: error.message || "Unable to login with Facebook.",
        icon: "error",
        confirmButtonText: "Ok",
      });
    }
        ...
        <div className="shadow-xl max-w-xl w-full mx-auto mt-10 lg:mt-0 p-4 md:p-10 rounded-lg border bg-white">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-7">
            ...
            <p>
              ...
              <Link to={"/login"} className="text-[#C480CF] font-bold underline">
                Login
              </Link>
            </p>
            <button
            ...
            >
              <span>
                {loader ? (
                  <p className="border-t rounded-xl border-white border-solid w-4 h-4 animate-spin"></p>
                ) : (
                  "Create Account"
                )}
              </span>
